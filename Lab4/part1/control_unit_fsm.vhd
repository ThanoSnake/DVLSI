...
begin
        ...
        if rst_n = '0' then
            ...
            case current_state is
                ...
                when ST_NORMAL =>
                    ...
                    if counter = N*N-2 and valid_in = '1' then
                        current_state <= ST_DRAINING;
                    end if;


                when ST_DRAINING =>
                    if counter = N*N+2*N+2 then
                        current_state <= ST_IDLE;
                        counter <= 0;
                    else
                        counter <= counter + 1;
    ...
    rd_en(1) <= '1' when ((counter >= 2*N-1 and valid_in = '1') or current_state = ST_DRAINING) else '0';
    wr_en(2) <= '1' when ((counter >= 2*N and valid_in = '1') or current_state = ST_DRAINING)  else '0';

    rd_en(2) <= '1' when ((counter >= 3*N-1 and valid_in = '1') or current_state = ST_DRAINING) else '0';
    ...
    valid_out       <= '1' when (counter >= 2*N+3 and counter <= N*N+2*N+2 and valid_in_del = '1') else '0';
    image_finished  <= '1' when counter = N*N+2*N+2 else '0';
